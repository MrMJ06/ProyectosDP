...
public interface TripRepository extends JpaRepository<Trip, Integer> {
	 ...
	 * 
	 */
	@Query("select sum(case when(t.lusits.size >0) then 1.0 else 0.0 end)/count(t)  from Trip t")
	String getRatioTripsWithLusits();

	@Query("select m from Trip t join t.managers m where t.lusits.size = (select max(tp.lusits.size) from Trip tp) group by m")
	Collection<Manager> getManagerFromTripsWithMoreLusits();

	@Query("select t from Trip t join t.managers m where m.lusits.size = (select max(mn.lusits.size) from Manager mn) group by t")
	Collection<Trip> getTripsFromManagerWithMoreLusits();

	@Query("select l  from Trip t join t.lusits l where t.id=?1 and (l.moment < NOW())")
	Collection<Lusit> findPublicatedLusitsFromTrip(int tripId);
	/*****/
	...
	@Query("select sum(case when(t.cancelReason is not null and t.cancelReason != \'\') then 1.0 else 0.0 end)/count(t)  from Trip t")
	...
	Collection<Trip> findTripSurvivalApplication(int idExplorer, int idSurvivalClass);
