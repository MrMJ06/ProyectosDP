	...
	public ModelAndView save(@Valid final Sponsorship sponsorship, final BindingResult binding) {
		...
		if (binding.hasErrors())
			...
			if (sponsorship.getCreditCard().getExpirationYear() < currentDate.getYear() % 100)
				return this.createEditModelAndView(sponsorship, "sponsorship.expired.error");
			else if (sponsorship.getCreditCard().getExpirationYear() == currentDate.getYear() % 100)
				if (sponsorship.getCreditCard().getExpirationMonth() < currentDate.getMonth() % 100)
					return this.createEditModelAndView(sponsorship, "sponsorship.expired.error");
			try {
