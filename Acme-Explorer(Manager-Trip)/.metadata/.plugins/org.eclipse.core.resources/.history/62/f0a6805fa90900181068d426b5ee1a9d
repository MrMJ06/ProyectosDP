...
<br />
<jstl:if test="${not empty alerts}">
	<h2>
		<spring:message code="detailed.trip.alerts" />
	</h2>
	<display:table name="alerts" id="alert">
		...
		<spring:message code="detailed.trip.alert.title" var="alertTitle" />
		<spring:message code="detailed.trip.alert.description"
			var="alertDescription" />
		<display:column property="title" title="${alertTitle}" class="${classTd}"/>
		<display:column property="description" title="${alertDescription}" class="${classTd}"/>
	</display:table>
...
<security:authorize access="hasRole('MANAGER')">
	<jstl:if test="${hasManager and trip.startDate>currentDate and (trip.cancelReason==null || trip.cancelReason==\"\")}">
		<a href="alert/manager/create.do?tripId=${trip.id}">
			<button class="btn">
				<spring:message code="detailed.trip.alert.create" />
			</button>
		</a>
	</jstl:if>
</security:authorize>
