...
public class SurvivalClassService {
	...
	public SurvivalClass save(final SurvivalClass survivalClass) {
		...
		if (LoginService.getPrincipal().getAuthorities().contains(authority)) {
			...
			for (final Trip trip : this.tripService.findTripApplicationSurvival(survivalClass)) {
				Assert.isTrue(this.applicationService.getApplicationTripExplorer(this.actorService.findActorByUserAccountId(LoginService.getPrincipal().getId()).getId(), trip.getId()).getStatus().equals("ACCEPTED"));
				break;
			}
		}
