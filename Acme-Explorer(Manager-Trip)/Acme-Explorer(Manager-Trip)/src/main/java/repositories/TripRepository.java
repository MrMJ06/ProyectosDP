...
public interface TripRepository extends JpaRepository<Trip, Integer> {
	 ...
	 * 
	 */
	@Query("select sum(case when(t.alerts.size >0) then 1.0 else 0.0 end)/count(t)  from Trip t")
	String getRatioTripsWithAlerts();

	@Query("select m from Trip t join t.managers m where t.alerts.size = (select max(tp.alerts.size) from Trip tp) group by m")
	...
	Collection<Alert> findPublicatedAlertsFromTrip(int tripId);
