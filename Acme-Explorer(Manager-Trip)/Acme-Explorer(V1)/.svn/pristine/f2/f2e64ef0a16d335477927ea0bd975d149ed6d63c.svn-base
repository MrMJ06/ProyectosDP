...
public class ActorController extends AbstractController {
	...
	public ModelAndView registerExplorer(@ModelAttribute("actor") @Valid final Explorer explorer, final BindingResult binding, @RequestParam("confirmPassword") final String confirmPassword) {
		ModelAndView result;
		Authority auth;
		...
		if (binding.hasErrors())
			...
			try {
				...
				Assert.isTrue(confirmPassword.equals(explorer.getUserAccount().getPassword()), "Passwords do not match");
				this.actorService.registerExplorer(explorer);
				result = new ModelAndView("redirect:/welcome/index.do");
			} catch (final Throwable oops) {
				if (oops.getMessage().contains("Passwords do not match"))
					result = this.createEditModelAndView(explorer, "actor.params.confirm.error");
				else
	...
	public ModelAndView registerRanger() {
		...
		result.addObject("actionURL", "actor/register-ranger.do");
	...
	public ModelAndView registerRanger(@ModelAttribute("actor") @Valid final Ranger ranger, final BindingResult binding, @RequestParam("confirmPassword") final String confirmPassword) {
		ModelAndView result;
		Authority auth;
		...
		if (binding.hasErrors())
			...
			try {
				...
				Assert.isTrue(confirmPassword.equals(ranger.getUserAccount().getPassword()), "Passwords do not match");
				this.actorService.registerRanger(ranger);
				result = new ModelAndView("redirect:/welcome/index.do");
			} catch (final Throwable oops) {
				if (oops.getMessage().contains("Passwords do not match"))
					result = this.createEditModelAndView(ranger, "actor.params.confirm.error");
				else
